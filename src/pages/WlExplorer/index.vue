  ...
  <div class="wl-explorer">
    ...
    <el-form class="wl-header-btn" :inline="true" size="medium" @submit.native.prevent>
      <el-form-item>
        ...
        <submit-btn type="danger" @btn="wlDownload" :status="load.upload">
          下载文件
        </submit-btn>
        <!-- solt自定义头部按钮区 -->
...
<script>
import submitBtn from '../../components/submit-btn'; // 导入防抖组件
  ...
  methods: {
    ...
    filePathChange(item){
      if(!this.matched_path){
        this.$confirm(`文件管理器内找不到"${this.file.path}"。请检查拼写并重试。`,'文件资源管理器', {
          confirmButtonText: "确定",
          callback: ()=>{},
